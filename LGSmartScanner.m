		...
		if (error != kIOReturnSuccess) {
			...
			for (currentAttributeIndex = 0; currentAttributeIndex < kSMARTAttributeCount; currentAttributeIndex++) {
				...
				if (currentAttribute.attributeId == kWindowSMARTVertexAvgWriteCount) {
					...
					wearTitle = [NSMutableString stringWithFormat: @"Wear: %@", [self.wearCount stringValue]];
				...
				if (currentAttribute.attributeId == kWindowSMARTVerteAvailableLife) {
					...
					NSMutableString *lifeTitle;
					lifeTitle = [NSMutableString stringWithFormat: @"Life: %@%%", [self.lifePercent stringValue]];
					[driveLifeIndicator setTitle:lifeTitle];	
				}
			}
		}
	}	
...
- (void)initializeSystemTray {
	...
	driveNormal = [[NSImage alloc] initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"vertex-logo-24" ofType:@"png"]];
	driveError = [[NSImage alloc] initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"vertex-logo-24" ofType:@"png"]];
	driveHighlight = [[NSImage alloc] initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"vertex-logo-24" ofType:@"png"]];
	...
	driveLifeIndicator = [trayMenu addItemWithTitle: @"Life:"  
									  ...
									  keyEquivalent: @""];
	driveAverageEraseCount = [trayMenu addItemWithTitle: @"Erase:"  
												 action: @selector (doNothing)  
										  keyEquivalent: @""];
	[trayMenu addItem:[NSMenuItem separatorItem]];
	[trayMenu addItemWithTitle: @"Open Forum"  
						action: @selector (openForumURL)  
				 keyEquivalent: @"f"];
	...
	[trayItem setAlternateImage:driveHighlight];
	[trayItem setImage:driveNormal];
	[trayItem setTitle:@""];
	[trayItem setMenu:trayMenu];
	[trayItem setToolTip:@"Vertex Monitor"];
...
- (void)openForumURL {
	...
	[[NSWorkspace sharedWorkspace] openURL:[NSURL URLWithString:@"http://www.ocztechnologyforum.com/forum/forum.php"]];
}
